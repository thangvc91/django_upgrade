...
@csrf_exempt
def doc2html1(request):
    return render(request, 'doc2html1.html')
@csrf_exempt
...
def forgot(request):
    if request.method == 'POST':
        ...
        if not pwds:
            ...
            return render(request,'posts/forgot.html', context)     
        else:
            # messages.info(request, 'Your password has been sent successfully to your email , please check!')
            # return HttpResponse({f'{pwds.batpassword}': f'{pwds.batpassword}'}, status=500) 
            # sendemail(email,pwds.batpassword)
            context = {
            'info' : "Your password has been sent successfully to your email , please check!"
            }
            return render(request,'posts/forgot.html', context)       
    else:       
